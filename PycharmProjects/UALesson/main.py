...
# TODO:
In the Developer subclass a variable with a list of languages
...
# from Lesson8.UserProfile import *
import dob as dob

from UserProfile import Designer, Developer
...
def calculate_expected_salary(num_of_exp_years, usr_information, sw_programs, number_of_edu_years):
    try:
...
while create_a_candidate == True:
    try:
        for candidate in valid_candidate:
            print(candidate)
        candidate = input("Are you a Developer or a Designer? ")
        print("\n")
        ...
        number_of_education_years = input("Please enter the number of years you have been learning to code/design: ")
        print("\n")
        ...
        if candidate == "Developer":
            calculate_expected_salary(user_exp, usr_info, usr_coding_langs, number_of_education_years)
            user_developer = Developer(dobString, full_name, country, state, number_of_education_years, age, usr_coding_langs)

            user_password = user_developer.get_password()          # this will contain default password
            print("Your initial password was: ", user_password)  # output of default password
            print("\n")
            ...
            user_developer.set_password(new_password)      # this will set the value of new password in class
            user_password = user_developer.get_password()  # this will retrieve the new user input for password
            print("Your new password is: ", user_password) # output of new password
            print("\n")

            user_id = user_developer.create_unique_id()
            print("Your confirmation Id is ", user_id)
            print("\n")

        if candidate == "Designer":
            calculate_expected_salary(user_exp, usr_info, sw_pkgs, number_of_education_years)
            user_designer = Designer(dobString, full_name, country, state, number_of_education_years, age, sw_pkgs)
            user_password = user_designer.get_password()  # this will contain default password
            print("Your initial password was: ", user_password)  # output of default password
            print("\n")
            ...
            user_designer.set_password(new_password)  # this will set the value of new password in class
            user_password = user_designer.get_password()  # this will retrieve the new user input for password
            print("Your new password is: ", user_password)  # output of new password
            print("\n")

            user_id = user_designer.create_unique_id()
            print("Your confirmation Id is ", user_id)
            print("\n")
